# Olorin — Engineering Journal (Archive 1: 2026-07-06 → 2026-07-12)

This is the first archive of the running build log — split off on
2026-07-12 once the live `ENGINEERING_JOURNAL.md` passed ~1000 lines,
purely to keep each file readable; nothing here is superseded. Continue
reading forward in `ENGINEERING_JOURNAL.md`, which picks up from
2026-07-12 (continued) onward. Full architectural rationale still lives
in `OLORIN_PROJECT.md`.
...
## 2026-07-11 — V2: episodic memory (write path + query path), one real bug found and fixed

### `memory/history.py` built — Section 10's `conversations` table, live
- Schema exactly per Section 10: `timestamp`, `repo_path`, `user_query`,
  `agent_response`, `tools_used` (JSON array), `backend_used`,
  `complexity_score`.
- `log_conversation()` / `get_recent()` / `search_history()`. Connection
  pattern (`sqlite3.connect(config.MEMORY_DB_PATH)`, schema created on
  connect) mirrors `memory/cache.py` — same file, same DB, different
  table, consistent with the single `memory.db` already in use.
- **Scope decision, stated up front:** `search_history()` is
  keyword-overlap search (shared-word count between the query and each
  row's stored text), not semantic/embedding search. A proper semantic
  history search needs its own vector-store design (second Chroma
  collection? reuse the code collection with a type filter?) — a bigger
  decision than this pass warrants. Keyword overlap is a real, useful
  first cut; revisit with embeddings only if keyword recall proves too
  weak in practice.

### `tools/history.py` + `tools/registry.py` — `search_history` wired as a fourth tool
- Same shape as `tools/search.py`: honest `{"error": ..., "results": []}`
  when nothing's been logged for a repo yet, rather than a silent empty
  list.
- `repo_root` injected by `execute_tool()`, same as every other tool —
  never trusted from model-supplied arguments.
- System prompt (`core/agent.py`'s `build_system_prompt()`) updated to
  describe all four tools, including when to reach for `search_history`
  ("what did I figure out about X before?"-style questions).

### `providers/base.py` + `core/llm_client.py` — `complexity_score` threaded onto `ProviderResponse`
- `ProviderResponse` gained `complexity_score: int | None`, set by
  `LLMClient._route()` in auto mode (both the Groq-success and
  local-fallback branches), immediately after the provider call returns —
  providers themselves have no visibility into routing, only into
  answering, so this can't be set inside `GroqProvider`/`OllamaProvider`.
- Left `None` for forced-provider calls (`--provider X`, `--force-local`)
  and cache hits, where the scorer never actually runs — an honest gap,
  not a guess.

### `core/agent.py` — `Agent.run()` wired to log every completed run
- Accumulates `tools_used` (tool names, in call order, duplicates kept —
  same style as the Engineering Journal's own "search_codebase (4x)"
  session summaries) and tracks `last_response` across the loop.
- `_log_conversation()` called at **every** return point — clean finish,
  the defensive unexpected-`finish_reason` branch, and max-steps-reached
  — not just the happy path, so a run that goes wrong still leaves a
  record of what was asked and what happened.
- `backend_used` derivation handles the ollama/groq asymmetry correctly:
  both local personas report `provider="ollama"` (see
  `providers/ollama_provider.py`), so `response.model` (`"boromir"` /
  `"faramir"`) is used for local; `response.provider` (`"groq"`) is used
  for cloud, since Groq's model name isn't the label Section 10's schema
  wants.
- Logging wrapped in try/except — a logging failure must never break the
  actual answer the user is waiting on.

### `cli.py` — added `history` command
- `python cli.py history [--path] [--limit] [--search]` — thin read-only
  wrapper over `memory/history.py`, added purely so the write path could
  be verified live without a raw `sqlite3` shell.

### Real bug found + fixed: post-retrieval complexity scorer never actually saw retrieved content
- **Symptom:** first live verification of episodic memory logged
  `complexity_score=0` on every row, including a run where
  `search_codebase` had already returned 5 real chunks by the time the
  final answer was produced.
- **Root cause:** `Agent.run()` called `self.llm.chat(...)` without ever
  passing `retrieved_chunks` — on any step, including step 2+ after tool
  results were already sitting in the message history. `score_complexity()`
  (`core/llm_client.py`) always scored against an empty list as a result.
  Section 7's headline design goal — "classify complexity AFTER seeing
  retrieved chunks, not before, since the retrieved context is the real
  signal" — had genuinely never been exercised in the live `ask` path;
  it had only ever been proven in `llm_client.py`'s standalone smoke test
  with hardcoded chunks. Pre-existing since V1, surfaced now because
  episodic memory logging made the always-zero score visible and
  permanent for the first time.
- **Fix:** added `Agent._accumulate_retrieved_chunks()`, called after
  every tool execution in the loop, appending `search_codebase` chunk
  content and `read_file` content into a running `retrieved_chunks` list
  that's threaded into every subsequent `llm.chat()` call in the same run.
  `list_files` and `search_history` results are deliberately excluded —
  filenames and past conversations aren't the code-context signal this
  scorer is meant to weigh, and including them would conflate two
  orthogonal concerns.
- **Verified live:** `ask "How is Rust being used in project?"` — step 1
  (pre-retrieval) still correctly scored low; `search_codebase` returned
  5 chunks; step 2's `llm.chat()` call scored `complexity_score=42
  > COMPLEXITY_THRESHOLD (15)`, and the router escalated to Groq for the
  first time based on real retrieved-content volume rather than query
  wording alone. `python cli.py history` confirmed the logged row shows
  `backend=groq complexity=42`, not a guessed or stale number.
- **Side effect flagged, not a regression:** queries that pull back a lot
  of `search_codebase` content will now legitimately escalate to Groq
  more often than V1's routing numbers suggested, since the scorer is
  finally seeing real retrieval volume. Worth watching Groq usage against
  the free-tier budget (Section 5) going forward, not something to patch.

### Verified live, end-to-end
- `ask "how does the agent loop handle tool calls?"` → logged correctly,
  `backend=boromir`, `tools=search_codebase`.
- `history` and `history --search "tool calls"` → both return the
  logged row correctly.
- `ask "have I asked about the agent loop before?"` → agent chose
  `search_history` over `search_codebase` **on its own**, retrieved the
  actual prior conversation, and gave a grounded answer that correctly
  references the earlier question rather than hallucinating a summary —
  the real test of whether the tool is useful, not just wired up.
- `ask "How is Rust being used in project?"` → confirmed the
  retrieved_chunks fix live (see above), correctly escalated to Groq,
  logged `complexity=42`.

### Status
- [x] `memory/history.py` — episodic memory write path (Section 10),
  verified live
- [x] `search_history` query path — keyword-overlap search, wired as a
  fourth agent tool, verified live (agent chose it correctly on a
  memory-shaped question)
- [x] `complexity_score` threaded through `ProviderResponse` →
  `LLMClient._route()` → episodic memory log
- [x] `cli.py history` command for live verification/browsing
- [x] Real bug found and fixed: post-retrieval complexity scorer never
  received real retrieved_chunks in the live agent path — fixed via
  `_accumulate_retrieved_chunks()`, verified live with a real Groq
  escalation driven by real chunk volume
- [ ] Next: remaining V2 features (OLORIN.md per-repo config loading,
  Engineering Journal automation, entity memory) — per Section 11's V2
  tier

---

## 2026-07-11 (continued) — Knowledge Capsules, pulled forward from V3

### Design decision: entity memory deferred, Knowledge Capsules built instead
- Discussed entity memory's design gap directly (Section 10's `entities`
  table was a stub with no resolved population mechanism, type scope, or
  update policy — unlike episodic memory, which had a fully specified
  schema before any code was written) versus Knowledge Capsules, listed
  under V3 but with a clearer value proposition and less open design
  risk.
- **Decision: build Capsules now, defer entity memory.** Same judgment
  as the daemon deferral — recognizing a lower-value/higher-uncertainty
  item and choosing the better-specified, higher-payoff one out of
  strict roadmap order, logged with the same rigor as any other
  decision rather than silently reordered.

### Design conversation: context budget
- Boromir/Faramir run at 16K ctx (Modelfile-set, independent of the
  underlying models' larger native max context). The system prompt is
  resent in full on every step of the ReAct loop (no persistent
  server-side context between calls), and `read_file` alone can return
  up to ~5,000 tokens in a single call — so the real risk wasn't repo
  size, it was capsule text competing with growing tool-result content
  for the same fixed budget, worst exactly on the multi-step queries
  where the budget is already tightest.
- **Resolved design:** inject capsules into the system prompt for step 1
  only (when the model is deciding whether/what to call, which is the
  only point capsules actually help with), then rebuild `messages[0]`
  without them before step 2 — capsules are dead weight once the model's
  reasoning over concrete retrieved content instead of trying to orient
  itself. Paired with a hard length cap enforced in the generation
  prompt itself ("3-4 sentences, under 100 words") so the step-1 tax is
  small and predictable regardless of module size.
- **Deliberately deferred:** per-query relevance filtering (only inject
  capsules for modules that seem related to the question) — the actual
  answer for large repos, but solving a problem this repo's scale
  (5-6 modules) doesn't have yet. Same discipline as the daemon
  deferral: revisit once a repo is big enough that even step-1-only,
  length-capped capsules start crowding the budget.

### `memory/capsules.py` + `indexer/capsules.py` — store/generate split
- Storage (`memory/capsules.py`): `capsules(repo_path, module, summary,
  source_files, last_updated)`, upsert-by-`(repo_path, module)`,
  `needs_regeneration()` comparing the current (path, hash) set per
  module against what the stored capsule was generated against — same
  hash-comparison philosophy as `cli.py`'s per-file reindex-skip logic,
  applied at module granularity.
- Generation (`indexer/capsules.py`): takes an LLM client as a plain
  duck-typed argument rather than importing `core.llm_client` directly —
  `indexer/` has had no dependency on `core/` up to this point, and
  there was no reason to introduce one; `cli.py`, which already
  orchestrates across both, wires them together.
- **Faramir generates capsules, not Boromir** — Section 2's lore
  assigns Faramir to general reasoning/synthesis and Boromir to fast
  task-focused tool execution; summarizing "what does this module do" is
  squarely the former. The known Faramir risk logged elsewhere
  (`<think>` blocks confusing tool-call parsing) doesn't apply here —
  capsule generation makes no tool calls, it's a single plain
  completion.
- Module grouping uses a dedicated extension allowlist in `cli.py`
  (`_CODE_EXTENSIONS`) rather than trusting the walker's `language`
  field — keeps capsule generation decoupled from `chunker.py`'s
  internal language-support matrix, which this feature has no need to
  know about. Loose root-level files (`config.py`, `cli.py`) fall under
  a synthetic `"root"` module.

### `cli.py index` — capsule generation wired in after the existing pipeline
- Runs after the walk/chunk/embed/store loop completes: groups files by
  module, checks `needs_regeneration()` per module, regenerates via
  Faramir only for modules whose file-hash set actually changed.
- Wrapped in try/except per module — a capsule generation failure
  (Ollama down, model error) must not lose the indexing work that
  already succeeded above it.

### `core/agent.py` — step-1-only injection implemented
- `build_system_prompt(repo_root, include_capsules=True)` — capsule
  section built from `memory.capsules.get_all_capsules()`, wrapped in
  try/except (a capsule read failure shouldn't block answering).
- `Agent.run()`: initial `messages[0]` includes capsules; immediately
  after step 0's tool-call handling, `messages[0]` is unconditionally
  rebuilt with `include_capsules=False` before the loop continues to
  step 2. Guarded by `if step == 0` so this only fires once per run.

### Verified live, end-to-end
- `python cli.py index .`: 30 files walked, 16 (re)indexed, 7 capsules
  generated (`tools`, `memory`, `core`, `providers`, `root`, `indexer`,
  `indexer_core`) at 12-26s each (Faramir `think:true` — deliberation is
  the point, cost is acceptable since this runs at index time, off the
  interactive critical path).
- Re-running `index` immediately after: `Capsules: 0 generated, 7
  unchanged` — confirmed the hash-based skip logic works, not just the
  generation path.
- `ask "what is this project and how is it structured?"` — answered in
  a **single step, zero tool calls**, correctly synthesizing all seven
  modules' roles from capsule text alone (correctly distinguished
  `memory/capsules.py` storage from `indexer/capsules.py` generation —
  real synthesized understanding, not a lucky guess). This is the
  feature's actual payoff: an orientation-style question resolved
  without a single exploratory `list_files`/`read_file` round-trip.
- Re-ran the same style of query warm (Boromir already loaded):
  13.3s total, confirming the earlier 40s `ask` was Ollama's cold-start
  model-swap cost (already measured at ~41s back on 2026-07-06), not
  capsule injection overhead — capsules are not meaningfully adding to
  per-call latency.
- `ask "what does the score_complexity function do, read the actual
  code"` — forced a real multi-step run: step 1 (capsules present) →
  `search_codebase` → step 2 (capsules correctly stripped from
  `messages[0]`, no malformed history, no crash) → `read_file` → step 3
  → final answer from Groq, accurate and grounded in the real code.
  This is the step-1-only stripping logic actually exercised under real
  multi-step load for the first time, and it held up cleanly. Also
  reconfirmed the retrieved_chunks fix from earlier today (real Groq
  escalation, driven by real chunk volume) still works correctly
  alongside capsule injection.

### Status
- [x] `memory/capsules.py` + `indexer/capsules.py` — store/generate
  split, verified live
- [x] `cli.py index` — capsule generation + hash-based skip, verified
  live (both the generate path and the skip path)
- [x] `core/agent.py` — step-1-only capsule injection, verified live
  under both a single-step and a real multi-step run
- [x] Zero new bugs found this session — first V2 feature session
  without one, worth noting for contrast with episodic memory's session
- [ ] Next: remaining V2 features (OLORIN.md per-repo config loading,
  Engineering Journal automation, entity memory) — per Section 11's V2
  tier
...
## 2026-07-11 (continued) — OLORIN.md loading, a real list_files bug, and index-on-demand

### OLORIN.md per-repo config loading — shipped
- `core/agent.py`'s new `load_olorin_md()`: reads `OLORIN.md` from the
  repo root once per `Agent` instance, honest `None` when absent, capped
  at 4,000 chars defensively.
- **Deliberately never stripped**, unlike Knowledge Capsules — this is
  explicit user-authored instruction ("never suggest SQLAlchemy"), not
  orientation help the model can safely stop needing once it's grounded
  in retrieved code. Threaded through both the step-1 and step-2+
  `build_system_prompt()` calls in `Agent.run()`.
- Wrote a real `OLORIN.md` for this repo (project conventions, persona
  roles, where provider quirks belong) as both the working example the
  doc always promised and the live test fixture.
- **Verified live, two ways:** a query about refactoring the agent loop
  correctly cited the "hand-written ReAct, not LangChain" rule from
  OLORIN.md on step 1; a forced multi-step query (`read_file` → Groq)
  correctly cited the "provider quirks belong in ollama_provider.py" rule
  on its **step 2 answer**, confirming persistence past the point where
  capsules get stripped — the actual behavioral difference this design
  was built for, not just presence-of-a-section.

### Real bug found + fixed: `list_files` had no result-size cap (bug #9)
- **Symptom, found while verifying OLORIN.md on a real repo
  (Vorticity):** `ask "what is this project?"` on an unindexed repo took
  115 seconds on its final step. Root cause wasn't OLORIN.md — it was
  `list_files({'pattern': '*'})` returning an entirely unbounded file
  list (Next.js build output, ML checkpoints, notebook checkpoints, none
  of it excluded) that the local model then had to process in full
  before answering.
- **Notable interaction with the complexity scorer:** `list_files`
  results are deliberately excluded from `Agent._accumulate_retrieved_
  chunks()` (filenames aren't code-context signal, see bug #8's fix) —
  which means an oversized `list_files` result never had a chance to
  correctly escalate to Groq. It just silently cost real local latency
  instead, invisible to the one system designed to catch "this query got
  expensive."
- **Fix:** `tools/reader.py`'s `list_files()` gained a `max_files=200`
  hard cap (mirroring `read_file`'s existing `max_chars` pattern) plus
  `total_matched`/`truncated` fields so the agent knows when to narrow
  its pattern instead of assuming it saw everything. Expanded
  `IGNORED_DIRS` to include `.next`, `dist`, `build`, `target`,
  `.ipynb_checkpoints`, `.pytest_cache`, `.mypy_cache` — the actual bulk
  of the 115s repro.
- **Verified live:** re-running the original repro's `list_files` call
  came back in ~15s total (both steps) instead of the original single
  115s step; a targeted `*.py` pattern query on the same repo returned a
  clean 9-file list in under 15s combined.

### Design conversation: indexing shouldn't be a step the user has to remember
- Raised directly: for a product Olorin is meant to become, requiring a
  separate manual `index` before every new repo's first `ask` is exactly
  the kind of implementation detail ("think in terms of embeddings and
  vector databases") a real user shouldn't have to know exists. Users
  think in terms of projects and questions.
- **Resolved design, confirmed directly:** auto-indexing should NOT mean
  "reindex everything on every call" — the hash-based skip logic already
  built (Section 8) means checking freshness on every `ask` is cheap when
  nothing changed (a fast walk + hash comparison, no re-chunk/re-embed/
  re-capsule work), and only pays real cost when something's actually
  different. This reframes indexing from an explicit user-owned step to
  an automatic, lazy-but-always-checked precondition of every query —
  sometimes called index-on-demand or self-indexing RAG. The retrieval
  architecture itself (chunk → embed → vector search) doesn't change;
  only who owns freshness does.
- **Honest, not hidden, tradeoff:** because the daemon was deliberately
  deferred (V1.5), there's no background process to push first-time/
  changed-file indexing work off the interactive path — it has to
  happen inline, blocking, before the agent starts reasoning. This is a
  real cost the design doesn't eliminate, just makes automatic instead
  of manual. Addressed with explicit messaging rather than a silent
  hang: a "first time indexing—this may take a minute" message when the
  collection is empty, and a one-line "repo changed since last index"
  summary when real work happened on a repeat call — silence on the
  common case (nothing changed).

### `cli.py` refactored: `_index_repo()` extracted, wired into `ask`
- The full walk/hash-skip/chunk/embed/store/capsule pipeline, previously
  inline in the `index` command, is now `_index_repo(repo_path,
  verbose=True|False)`. `index` is now a thin wrapper
  (`verbose=True`, unchanged behavior/output). `ask` calls it
  (`verbose=False`) as an automatic precondition before constructing the
  `Agent`, checking `collection.count() == 0` beforehand to decide
  whether to print the first-time message.
- Added `--skip-index` as an explicit escape hatch (advanced/debugging
  use), consistent with the existing `--provider`/`--force-local`
  pattern of giving explicit overrides rather than only ever inferring
  behavior.
- **Verified live, all three states on a real never-indexed repo
  (tutorial-video-generator):**
  1. First `ask` — printed the first-time message, indexed + generated
     7 capsules silently (no file-by-file spam), answered accurately in
     a single step from capsule text alone.
  2. Second `ask`, nothing changed — completely silent on indexing,
     single-step answer, notably *more* technically specific (correctly
     named WhisperModel, edge TTS, FFmpeg, headless Chromium) purely
     from capsule text — real implementation detail surviving the
     capsule length cap, not just generic descriptions.
  3. `--skip-index` — correctly skipped the freshness check with no
     message either way, while still correctly using the capsules
     already generated in step 1 (capsule injection reads storage
     independent of whether the freshness check ran).

### Status
- [x] OLORIN.md per-repo config loading — shipped, verified live
  (both presence-of-rule and persistence-past-step-1)
- [x] Real bug found and fixed: `list_files` had no result-size cap
  (bug #9), invisible to the complexity scorer by design — fixed with a
  hard cap + expanded noise-directory list, verified live
- [x] Indexing reframed from a manual step to an automatic, hash-
  cheap precondition of `ask` ("index-on-demand") — `_index_repo()`
  extracted and shared between `index` and `ask`, verified live across
  first-time/unchanged/skip-index states on a real never-indexed repo
- [ ] Next: remaining V2 features (Engineering Journal automation,
  entity memory once designed) — per Section 11's V2 tier

---

## 2026-07-12 — Direct-address routing, three real bugs it surfaced, one known limitation logged

### Direct-address routing override — shipped
- Raised directly: routing purely by the complexity scorer means there's
  no way to force a specific backend through phrasing alone, since the
  scorer is deliberately driven by retrieved-content volume, not query
  wording (Section 7). Wanted an explicit override expressed inline
  ("Groq, ...", "Boromir, ...") rather than only via the `--provider` flag.
- **Explicitly distinguished from a rejected idea:** Section 13 already
  rejected keyword-based *difficulty* classification. This is not that —
  it's a deterministic, narrow address parser functionally identical to
  the `--provider` flag, just inline. Nothing about routing is inferred;
  the user is stating a choice.
- `core/agent.py`'s new `parse_direct_address()`: matches only at the
  exact start of a query (`"Boromir, ..."`, `"Groq: ..."`, `"Olorin
  ..."`), maps onto the same `force_provider` strings the CLI already
  uses (`"olorin"`→`"groq"` per Section 2's lore — Olorin IS the Groq
  persona). Only applies when the caller didn't already force a provider
  — an explicit `--provider`/`--force-local` flag always wins. Strips the
  address from what reaches the model; keeps the original raw text
  (`original_query`) for episodic memory logging, since the user really
  did type it and it should stay searchable as what actually happened.
- **Verified live:** correct routing and stripping confirmed in logs for
  all three names; precedence (`--provider` beats an in-text address)
  confirmed separately; episodic memory confirmed to retain the original
  phrasing, not the cleaned version.

### Real bug found + fixed: forced-provider calls had no fallback (bug #10)
- **Symptom:** the very first live test of direct-address routing
  crashed the CLI outright — `ask "Groq, what is this project?"` hit a
  real Groq 413 ("Request too large", 39,218 tokens against a 12,000 TPM
  free-tier limit) and the exception propagated all the way up as a raw
  traceback. Auto-mode's Groq attempt has always been wrapped in
  `try/except ProviderError` with a fallback to local; the forced-`groq`
  and forced-local branches in `LLMClient._route()` never were. This gap
  predates direct-address routing (the `--provider groq` flag had the
  same hole) — it just went unnoticed because forcing Groq explicitly
  was rare until this session made it a natural, frequent thing to type.
- **Fix:** forced-`groq` now wraps the call in the same `try/except
  ProviderError` pattern as auto-mode, falling back to `self.
  default_local` (boromir) on any failure, tripping the circuit breaker
  if it was a rate limit. `cli.py`'s `ask` also gained a top-level
  try/except around `agent.run()` as a safety net for whatever a forced-
  local failure (e.g. Ollama not running) has no further automatic
  fallback for without contradicting the user's explicit choice — a
  clean red error message instead of a raw traceback either way.
- **Verified live under a genuinely new failure mode**, not just the
  original trigger: a later run hit a *different* Groq error entirely
  (a malformed tool-call, `400 tool_use_failed`, from `llama-3.3-70b-
  versatile` itself) and the fallback caught it exactly the same way —
  good evidence the fix is generically robust, not narrowly patched to
  one symptom.

### Real bug found + fixed: chunks had no size ceiling (bug #11)
- **Root cause:** `indexer/chunker.py`'s whole-file fallback (used for
  any language without a tree-sitter grammar — markdown, notably) has
  never had a size cap; the chunker's own docstring flagged this and
  explicitly deferred it to the embedder layer. Never implemented until
  now. `OLORIN_PROJECT.md` and `ENGINEERING_JOURNAL.md`, grown very
  large from this session's own documentation work, were the concrete
  trigger for bug #10's 413 — a single whole-file chunk large enough to
  blow a request's token budget on its own.
- **Fix:** `indexer/embedder.py`'s new `_split_oversized_chunks()`,
  called inside `embed_chunks()`, splits any chunk over `MAX_CHUNK_CHARS`
  (2000, roughly bge-small's own ~512-token limit — keeps what's
  embedded and what's stored/returned honestly in sync) into sequential
  sub-chunks. Deliberately a blunt line-budget split, not markdown/AST-
  aware — a generic fix that protects against any oversized chunk, not
  just markdown; logged explicitly as a known simplification, not a
  final answer (see the known limitation below).

### Real bug found + fixed: the chunk-split fix's own bug — duplicate IDs (bug #12)
- **Symptom:** the very next live test of bug #11's fix crashed
  indexing entirely — `ChromaDB DuplicateIDError`. `store.py`'s
  `_chunk_id()` hashes `file + lines`; the first version of
  `_split_oversized_chunks()` numbered each sub-chunk's lines relative
  to the CHUNK's own extracted text (always starting at 1), not the
  file's true absolute line numbers. Any two oversized chunks in the
  same file with roughly similar size would deterministically produce
  identical relative ranges ("1-55", "56-100", ...) — not a rare
  coincidence, a guaranteed collision for any file with 2+ chunks
  needing splitting.
- **Fix:** anchored each sub-chunk's line numbers to the original
  chunk's true starting line (parsed from its own `lines` metadata), so
  ranges reflect real file positions and stay genuinely unique. Also
  added a defense-in-depth guard in `store.py`'s `upsert_chunks()` —
  de-duplicates by ID before calling Chroma's `upsert()`, which
  otherwise aborts an ENTIRE file's upsert (every legitimate chunk in
  the batch, not just the colliding one) on a single collision. A
  backstop for any future collision cause, not a substitute for fixing
  root causes at the layer that produces them.
- **Verified live:** the guard fired correctly (`WARNING: 1 duplicate
  chunk id(s) collapsed before upsert`), indexing completed without
  crashing, and the subsequent `ask` call succeeded end-to-end.

### Known limitation logged, not fixed: retrieval miss on a real question
- **Symptom:** `ask "Boromir, summarize what OLORIN_PROJECT.md says
  about the daemon deferral"` answered that the document "does not
  directly discuss" the daemon deferral — false; Section 11's entire
  V1.5 tier and a dedicated Section 13 entry cover it extensively.
- **Assessment:** likely retrieval missing the right chunk after bug
  #11's line-budget split, not the model fabricating — notably it did
  NOT hallucinate a wrong daemon-deferral narrative, it honestly
  reported what it could and couldn't find, which is the safer failure
  mode. Plausibly exactly the caveat already logged in
  `_split_oversized_chunks()`'s own docstring: a blunt line-budget split
  can cut through a topical section instead of respecting document
  structure, hurting retrieval precision on long docs specifically.
- **Decision: logged as a known limitation, not investigated further
  this session.** A markdown/header-aware split would be the real fix
  if this proves to matter in practice — deliberately not chased now,
  consistent with the project's standing discipline of not building past
  what's actually been measured to be a problem.

### Status
- [x] Direct-address routing — shipped, verified live including
  precedence and episodic-memory-preserves-original-text behavior
- [x] Bug #10 (forced-provider no fallback) — fixed, verified live
  under two genuinely different Groq failure modes
- [x] Bug #11 (chunks had no size ceiling) — fixed
- [x] Bug #12 (chunk-split fix's own duplicate-ID bug) — fixed, with a
  defense-in-depth guard added alongside the root-cause fix
- [x] Known limitation logged: retrieval precision on long docs after
  line-budget splitting — deliberately deferred, not fixed
- [ ] Next: Engineering Journal automation, Together/Cerebras providers
  (per user direction, entity memory pushed further out — finishing-
  stage feature, not V2)

*(continued in `ENGINEERING_JOURNAL.md`)*
